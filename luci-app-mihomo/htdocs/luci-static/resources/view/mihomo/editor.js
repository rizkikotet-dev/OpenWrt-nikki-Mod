...
return view.extend({
  load: function () {
    return Promise.all([uci.load("mihomo"), mihomo.listProfiles()]);
  },
  render: function (data) {
    ...
    for (const profile of profiles) {
      o.value(
        mihomo.profilesDir + "/" + profile.name,
        _("File:") + profile.name
      );
    }
    ...
    for (const subscription of subscriptions) {
      o.value(
        mihomo.subscriptionsDir + "/" + subscription[".name"] + ".yaml",
    ...
    o.onchange = function (event, section_id, value) {
      return L.resolveDefault(fs.read_direct(value), "").then(function (
        content
      ) {
        m.lookupOption("mihomo.editor._file_content")[0]
          .getUIElement("editor")
          .setValue(content);
      });
    };
